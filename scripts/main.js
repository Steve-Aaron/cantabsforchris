$(function () {
  ...
  const updateCountdown = () => {
    const now = new Date(), voteDay = new Date('2025-07-09T10:00:00+0100');
    ...
    if (diff === 0) {
      ...
      $('#voting-countdown').text(now < end ? 'Voting is open now!' : 'Voting has ended');
    } else {
      $('#voting-countdown').html(`<span id="days-remaining">${diff}</span> days until voting`);
    }
  };
  updateCountdown();
...
$('.close-nav, .nav-list a').on('click', function () {
  ...
  $('.endorsement-slide').addClass('active').css({ position: 'relative', opacity: 1, visibility: 'visible' });
  ...
  $(window).on('scroll', function () {
    ...
    $('.section').each(function () {
      const $s = $(this), offsetY = $s.offset().top, h = $s.outerHeight();
      if (scrollY > offsetY - window.innerHeight && scrollY < offsetY + h && $s.hasClass('hero')) {
        const speed = parseFloat($s.data('speed')) || 0.1;
        $s.find('.hero-background').css('transform', `translateY(${(scrollY - offsetY) * speed}px)`);
      }
    });
  });
  ...
  $('#campaign-form').on('submit', function (e) {
    ...
    const apiKey = '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'; // replace server-side
